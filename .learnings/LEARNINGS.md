...
A later recovery established the complementary boundary: repeated new standalone e-paper refreshes prove that Linux and the application/display path are alive even when the host is absent from the LAN. In that case, inspect persistent networking before power or boot. On Raspberry Pi OS with Netplan and NetworkManager, a valid bootfs/cloud-init network document does not repair an existing instance when the rendered persistent profile has disappeared; cloud-init is not expected to render the same instance again. Restore one root-owned `0600` Netplan source from already validated local credentials, verify the generated NetworkManager profile has autoconnect and bounded-safe long-term retry semantics, and prove the repair with a real ordinary reboot. Preserve any paired GPU canary state and `tryboot.txt` during offline imaging. After pinned SSH returns, use the installed updater's status and rollback protocol until it reports `inactive`; never delete either file independently. Final health requires a new-boot proof set: automatic Wi-Fi association, successful boot recovery, ready endpoints, zero storage errors, and an untriggered physical display refresh at the configured cycle interval.
...
If the screen is visibly producing new images while LAN discovery fails, classify the problem as network-persistence-first and image the current card before any repair. Validate both filesystems on working copies, inject only the missing persistent network source, apply only a preimage-verified sector patch, and read back every changed sector. Then cold-boot once, wait for pinned SSH and HTTP readiness, complete any updater-owned canary transaction through the updater itself, and observe at least one full natural display cycle without calling a refresh endpoint.

### Metadata
- Source: user_feedback
- Related Files: tools/epaperpod-deploy-zip.ps1
- Tags: epaperpod, runtime-audit, mdns, avahi, router, offline, last-activity, wifi-watchdog, zero-2-w, led, evidence-boundary, netplan, networkmanager, tryboot, sd-recovery

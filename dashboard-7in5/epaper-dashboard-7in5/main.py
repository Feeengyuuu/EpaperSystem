...
if os.name != "nt":
    try:
        ...
        logging.getLogger(__name__).debug("Optional hardware/service import skipped: %s", e)
        pass
...
def update_data_thread():
    ...
    while True:
        ...
        if now - data_store.last_update['gmail'] > 300:
            try:
                creds = None
                if Credentials and Request and os.path.exists(GMAIL_TOKEN_PATH):
                    creds = Credentials.from_authorized_user_file(GMAIL_TOKEN_PATH, GMAIL_SCOPES)
                    if creds and creds.expired and creds.refresh_token:
                        creds.refresh(Request())
                        with open(GMAIL_TOKEN_PATH, 'w') as t: t.write(creds.to_json())
                if build and creds and creds.valid:

...
def test_game_calendar_setting_and_refresh_only_saves_normalize_cadence(plugin_env, enabled_setting, refresh_only):
    request_data = ({"refresh_settings": json.dumps({"refreshType": "interval", "unit": "hour", "interval": 2})}
                    if refresh_only else {enabled_setting: "true"})
    plugin_env.inner_manager.add_plugin_to_playlist_snapshot("Default", {
        ...
        "plugin_settings": {enabled_setting: "true"}, "refresh": {"interval": 7200},
    })

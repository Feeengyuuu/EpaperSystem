...
def canonical_theme(mode):
    palette = {
        "background": (245, 239, 226) if mode == "day" else (11, 13, 15),
        ...
        "accent": (187, 30, 45) if mode == "day" else (255, 77, 95),
    }
    return {
        "mode": mode,
        "requested_mode": "auto",
...
def test_box_office_theme_only_opposite_palette_reuses_warm_source_cache(
    monkeypatch,
    tmp_path,
):
    monkeypatch.setenv("INKYPI_BOX_OFFICE_CACHE", str(tmp_path))

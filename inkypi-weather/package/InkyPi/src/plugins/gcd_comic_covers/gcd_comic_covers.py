...
class GcdComicCovers(BasePlugin):
    ...
    def _write_cover_context(self, cover):
        series = str(cover.get("series_name") or "Comic").strip()
        number = str(cover.get("issue_number") or "").strip()
        date_label = str(cover.get("date_label") or "").strip()
        title = series if not number else f"{series} #{number}"
        summary = f"GCD comic cover: {title}"
        if date_label:
            summary += f" ({date_label})"
        ...
        write_context(
            "gcd_comic_covers",
            {
                ...
                "items": [{
                    "issue_id": cover.get("issue_id"),
                    "series": series[:120],
                    "issue_number": number[:60],
                    "date": date_label,
                    "page_url": cover.get("page_url"),
                    "image_url": cover.get("cover_url"),
                }],
            },
    ...
    def _candidate_order(self, candidates, state, today):
        ...
        exact = [candidate for candidate in candidates if candidate.get("match_quality") == "exact_day"]
        ...
        month_unseen = [
            ...
            if candidate.get("issue_id") not in exact_ids
        ]
        reset_last_issue_id = ""

        if not exact_unseen and not month_unseen:
        ...
        ordered = exact_unseen + month_unseen
    ...
    def _load_cover(self, candidate, dimensions, settings):
        ...
        if not cover_url:
            raise RuntimeError("GCD issue has no cover URL")
    ...
    def _candidate_date(self, candidate):
        for key in ["on_sale_date", "publication_date", "key_date"]:
    ...
    def _issue_cache_path(self, issue_id):
        ...
        return self._cache_dir() / "issues" / f"{issue_id}_{key}.json"

...
from .cs2_cards import followed_names, has_followed_team, parse_cs2_card, utc_datetime
...
def _fetch_feed(client, feed, settings, api_key, now, context):
    rows = []
    for page in range(1, MAX_PAGES + 1):
        result = client.request_json(
            "GET",
            "https://api.pandascore.co/csgo/matches/" + feed,
            context=context,
            max_bytes=MAX_BYTES,
            timeout=15,
            headers={
        ...
        if len(result.data) < 100 or sum(has_followed_team(row, settings) for row in rows) >= 4:
            break
    return rows
    ...
    for feed in FEEDS:
        ...
        try:
            ...
            if kind in {"AUTH", "LIMIT"}:
                ...
                break
    card = parse_cs2_card(_merge_snapshots(snapshots, ttls, now), tz, now, settings, game_logo_path=game_logo_path)
    source = _state(card, snapshots, updated)
